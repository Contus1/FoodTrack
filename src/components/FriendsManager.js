    ...
    async (query) => {
      ...
      try {
        ...
        const filteredResults = results.filter(
          (user) => !friendIds.has(user.id) && !sentRequestIds.has(user.id),
        );
  ...
  const handleSendRequest = async (userId) => {
    ...
    try {
      ...
      if (
        ...
        alert(
          "Friend request already sent or you are already friends with this user.",
        );
      } else {
      ...
      {searchResults.length > 0 && (
        <div className="space-y-2">
          {searchResults.map((user) => (
            <div
              key={user.id}
              className="flex items-center justify-between p-3 md:p-4 bg-white border border-gray-100 rounded-xl"
            >
              <div
