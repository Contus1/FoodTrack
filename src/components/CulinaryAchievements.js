...
const CulinaryAchievements = () => {
  ...
  const achievementDefinitions = [
    ...
    },
    {
      ...
      description:
        "Have an average rating between 3-4 stars across 30+ entries",
      category: "connoisseur",
      icon: "⚖️",
      rarity: "epic",
      condition: (stats) =>
  ...
  const calculateStats = () => {
    ...
    entries.forEach((entry) => {
      ...
      entry.tags?.forEach((tag) => {
        ...
        // Special dish types
        if (
          ["spicy", "hot", "chili"].some((spice) => tagLower.includes(spice))
        ) {
          stats.spicyDishes++;
        }
        if (
          ["dessert", "sweet", "cake", "ice cream", "chocolate"].some(
            (dessert) => tagLower.includes(dessert),
          )
        ) {
          stats.dessertCount++;
        }
      ...
      if (entry.location) {
        ...
        if (
          ...
          locationLower.includes("food truck") ||
          locationLower.includes("vendor")
        ) {
          stats.streetFoodCount++;
        }
        if (
          locationLower.includes("michelin") ||
          locationLower.includes("fine dining")
        ) {
          stats.michelinCount++;
        }
      }
    ...
    stats.avgRating =
      stats.totalEntries > 0 ? stats.avgRating / stats.totalEntries : 0;
  ...
  const calculateAchievements = useCallback(() => {
    ...
    const newAchievements = earnedAchievements.filter(
      (a) => a.earned && !localStorage.getItem(`achievement_${a.id}`),
    );
  ...
  const calculateProgress = (achievement, stats) => {
    ...
    if (achievement.id === "first_bite")
      return Math.min(stats.totalEntries, 1) * 100;
    if (achievement.id === "globe_trotter")
      return Math.min(stats.uniqueCountries / 5, 1) * 100;
    if (achievement.id === "five_star_critic")
      return Math.min(stats.fiveStarRatings / 20, 1) * 100;
    if (achievement.id === "social_butterfly")
      return Math.min(stats.friendsCount / 10, 1) * 100;
      ...
      {celebrationAchievement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-md p-4">
          <div className="glass-card rounded-3xl p-8 text-center max-w-sm transform animate-bounce shadow-2xl">
            ...
            <div
              className={`inline-block px-6 py-2 rounded-full text-white text-sm font-medium bg-gradient-to-r ${getRarityColor(celebrationAchievement.rarity)} shadow-lg`}
            >
        ...
        {filteredAchievements.map((achievement) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div
                className={`text-4xl ${achievement.earned ? "scale-110" : "grayscale"} transition-all`}
              >
                {achievement.icon}
              </div>
              <div
                className={`px-3 py-1 rounded-full text-xs font-medium text-white bg-gradient-to-r ${getRarityColor(achievement.rarity)} shadow-sm`}
              >
                {achievement.rarity}
              </div>
            </div>
            ...
            <div className="flex justify-between items-center pt-3 border-t border-gray-100">
              ...
              <span
                className={`text-sm font-semibold ${achievement.earned ? "text-orange-600" : "text-gray-400"}`}
              >
                {achievement.earned && "+"}
