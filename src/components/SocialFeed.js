              ...
              <span className="text-gray-900 font-medium">
                {taggedFriendNames
                  .map((friend, index) => {
                    ...
                    if (index === taggedFriendNames.length - 1)
                      return ` and ${name}`;
                    return `, ${name}`;
                  })
                  .join("")}
        ...
        <div className="flex items-center justify-between pt-3 mt-3">
          <div className="flex items-center space-x-2.5">
            ...
            >
              ...
              {entry.likesCount > 0 && (
                <span
                  className={`absolute -bottom-1 -right-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center text-[10px] font-medium rounded-full glass-badge shadow-sm ${
...
const SocialFeed = () => {
      ...
      try {
        ...
        if (dishIds.length > 0) {
          ...
          const entriesWithRatings = feedEntries.map((entry) => ({
            ...entry,
            communityRating: entry.dish_id
              ? communityRatings.get(entry.dish_id)
              : null,
          }));
  ...
  const handleSave = async (entryId) => {
    ...
    setEntries((prev) =>
      prev.map((e) => (e.id === entryId ? { ...e, isSaved: !e.isSaved } : e)),
    );
        ...
        <div className="text-center py-20">
          <div className="glass-panel rounded-2xl p-8 mx-4">
            ...
            <p className="text-gray-400 text-xs font-light">
              Follow friends to see their dishes here
            </p>
          </div>
        </div>
      ) : (
